...
sealed class Maybe<A>() {
    ...
    fun <A, B>flatMap(a: Maybe<A>, transform: (a: A) -> B): Maybe<B> = when(a){
        is Maybe.Nothing -> a as Maybe<B>
...
We've just implemented the Maybe monad in Kotlin.

Now, we could have achieve similar results in Java using a `final` class, but it would be impossible to guarantee exhaustiveness of pattern
matching, achieved using `when` expressions in Kotlin. This is where tests comes in of course.

We'll end with the implementation of another monad, the `Either` monad.

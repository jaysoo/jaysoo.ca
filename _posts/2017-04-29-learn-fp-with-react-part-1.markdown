...
In this three-part series, I want to take a more functional approach building React applications.

**Series contents:**

- Part 1 - Deconstructing the React Component
- Part 2 - Read-only context with Reader monad
- Part 3 - State management with Reducer and Component
...
Other objects are <code>String</code>, <code>Boolean</code>, etc.</p>
...
render(example1('alice'), document.querySelector('#root'))
{% endhighlight %}

This will render "*👏 Hello ALICE! 👏*" on the screen.

The function `example1` has two responsibilities:
...
Let's formalize these two concepts next.

## Look at my shiny new box!

We'll throw the original `Greeting` computation into a box, which I will call **View**.
...
// Throw in some computation, and let's call boxed value `Greeting`.
...
This is very tedious, so let's define a mapping function that can operate on our
boxed values.
...
Also, note that mapping a view does not do anything other than return a new box. All **computations
are delayed** until we call the `fold` function. This is a really useful property since we 
perform multiple transformations without needing to work with concrete values.
...
by mapping from box to box. You can extract an element at any point by folding it down, but now
...
so `f(g(x))` is the same as `compose(f, g)(x)`. The mathematical symbol "∘" denotes a composition --
.e.g. `f ∘ g === compose(f, g)`.
...
const identity2 = view => view.map(x => x)
{% endhighlight %}

Here, we have two mostly useless functions. When given a component or view,
both functions return something that is identical to the original argument.

{% highlight js %}
const MyComponent = () => <span>Hi</span>
const myView = View(MyComponent)

const Hoc = identity(MyComponent) // <Hoc/> === <MyComponent/>

const idView = identity2(MyView) // idView.fold() === myView.fold()
{% endhighlight %}

The difference between HOC and View is that the latter formalizes the concept of
...
that uses the `color` prop, *then* returns a component that is the original, but with the
...
Whereas our `identity` HOC is mapping over the resulting *value*, the `withColor` HOC is mapping
over the *input* props. Since `View.map` maps over the value (element),  there is no chance for us
to map over the original input.

So what can we do? Do we admit defeat?

Of course not. We just need to formalize this new concept!
...
Here, we have a type `Props` that is an object containing the color, and we the `Element` is the
...
Note that in `map` we call `f` with the resulting value, but with `contramap`
we call `g` with the input props, and then the result is passed to the computation.
...
Awesome! We can now map over results and contramp over inputs!

<div class="alert alert-info">
  <p><strong>A bit of theory:</strong>
  The View box is now also a <em>Contravariant</em>, in addition to being a functor. You can think of
  ...
  with the latter is reversed. In fact, we can call functors <a href="http://mathworld.wolfram.com/CovariantFunctor.html">Covariant Functors</a>,
  and contravariants <a href="http://mathworld.wolfram.com/ContravariantFunctor.html">Contravariant Functors</a>.
  But perhaps covariance/contravariance is a topic for another post.
  </p>

  <p>We can visualize the difference with these two pictures</p>
...
whether we are mapping over or input to decide which function we should call.

By formalizing these concepts we can be more explicit and precise about *what* we are mapping over, and *how* we are mapping.
Additionally, we have math laws to help us compose our program, which is awesoem!

From this point forward, I will no longer mention HOCs in this series since they are not needed.
...
1. Put a computation of the form `Props -> Element` into a box.
...
We can see that these arrays (`a`, `b`, `c`) are functors since they implement `map` that
obey the functor laws -- Feel free to verify this for yourself!
...
// We'll also define View.of that takes existing elements and wraps them in a box.
// The computation in this case just returns the element.
View.of = element => View(() => element)
{% endhighlight %}
...
Just for fun, we can also make View a *Monoid* by providing an `View.empty()` function.
...
both would result in just `centered` again.
...
Next up, we will replicate the React context using the Reader monad. So stay tuned!
